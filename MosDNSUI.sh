...
# 作者：ChatGPT
# 版本：3.2 (集成最终UI样式，修复Chart颜色联动，优化UI布局，提供自动刷新开关)
    ...
    <script>
        ...
        async function refreshData(forceRebuild = false, updateCharts = false) {
            ...
            if (forceRebuild) {
                ...
                mainContent.appendChild(document.querySelector('.system-info-container')); // Append system info card if not already
            }
